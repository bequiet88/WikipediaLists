...
public class LabelerDemo {
	public static void main(String[] args) {
		...
		try {
			String[] myArgs = args.length > 0 ? args : new String[] {
					"data", "human.labels",
					"sampleMixup/toyName.mixup" };
			...
			if (labelFile.exists())
				labelLoader.importOps(labels, base, labelFile);
			...
			if (mixupFile != null) {
				try {
					...
					System.out
							.println("couldn't mixup load file - are you sure "
									+ mixupFile.getName()
...
//		    Call teacher.train (learner) to create a trained extractor or classifier.
	...
	private static void usage() {
		System.out
				.println("usage: LabelerDemo directoryOfFilesToLabel labelFile [mixupFile]");
		System.out
				.println(" - for a demo, if you're in the minorthird/demos directory, try");
		System.out
				.println("java LabelerDemo sampleData/seminar-subset my.labels sampleMixup/toyName.mixup");
	}
}